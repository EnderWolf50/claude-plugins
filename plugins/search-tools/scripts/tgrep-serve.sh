...
# reap [session-id-to-ignore] [normalized-root-to-leave-alone] [only-this-normalized-root]
reap() {
  local skip="${1:-}" keep="${2:-}" only="${3:-}" snap live spid r
  snap="$(snapshot)"
  live="$(printf '%s\n' "$snap" | sed -n 's/^P|//p' | tr '\n' ' ')"
